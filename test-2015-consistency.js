...
// Volume statistics
const regularVolumes = regularData.map(d => d.volume);
const tickVolumes = tickVolumeData.map(d => d.volume);

console.log('\nVolume Statistics:');
console.log(`Regular volume range: ${Math.min(...regularVolumes)} to ${Math.max(...regularVolumes)}`);
console.log(`Tick volume range: ${Math.min(...tickVolumes)} to ${Math.max(...tickVolumes)}`);
console.log(`Regular volume avg: ${(regularVolumes.reduce((a, b) => a + b, 0) / regularVolumes.length).toFixed(6)}`);
console.log(`Tick volume avg: ${(tickVolumes.reduce((a, b) => a + b, 0) / tickVolumes.length).toFixed(2)}`);
